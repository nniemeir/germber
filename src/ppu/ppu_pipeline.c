...
bool window_visible(void) {
  return LCDC_WIN_ENABLE && lcd_get_context()->win_x <= 166 &&
...
void pipeline_load_window_tile(void) {
  ...
  if (ppu_get_context()->pfc.fetch_x + 7 >= lcd_get_context()->win_x &&
      ppu_get_context()->pfc.fetch_x + 7U <
          lcd_get_context()->win_x + YRES + 14U) {
...
void pipeline_push_pixel(void) {
  if (ppu_get_context()->pfc.pixel_fifo.size > 8) {
    ...
    u8 ly = lcd_get_context()->ly;
    u8 wx = lcd_get_context()->win_x;
    u8 wy = lcd_get_context()->win_y;

    bool use_window = window_visible() &&
                      (ppu_get_context()->pfc.line_x + 7 >= wx) && (ly >= wy);

    if (use_window) {
      // Window starts at wx, not affected by scroll_x
      if (ppu_get_context()->pfc.line_x + 7 >= wx) {
        int x = ppu_get_context()->pfc.pushed_x;
        ppu_get_context()->video_buffer[x + (ly * XRES)] = pixel_data;
...
void pipeline_process(void) {
  ...
  bool use_window = window_visible() &&
                    (ppu_get_context()->pfc.fetch_x + 7 >= wx) && (ly >= wy);

  if (use_window) {
    // Horizontal position relative to window start
    int win_relative_x = ppu_get_context()->pfc.fetch_x + 7 - wx;
    ...
    ppu_get_context()->pfc.map_x = win_relative_x;
    ppu_get_context()->pfc.map_y = ppu_get_context()->window_line;
    ppu_get_context()->pfc.tile_y = (ppu_get_context()->window_line % 8) * 2;
  } else {
    ppu_get_context()->pfc.map_y = ly + lcd_get_context()->scroll_y;
    ppu_get_context()->pfc.map_x =
        ppu_get_context()->pfc.fetch_x + lcd_get_context()->scroll_x;
    ppu_get_context()->pfc.tile_y =
        ((ly + lcd_get_context()->scroll_y) % 8) * 2;
  }
