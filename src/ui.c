...
void ui_init(int scale) {
  ...
  SDL_CreateWindowAndRenderer(XRES * scale, YRES * scale, 0, &sdlWindow,
                              &sdlRenderer);

  screen = SDL_CreateRGBSurface(0, XRES * scale, YRES * scale, 32, 0x00FF0000,
...
void update_dbg_window(int scale) {
  int xDraw = 0;
  int yDraw = 0;
  int tileNum = 0;
