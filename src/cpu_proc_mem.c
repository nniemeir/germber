...
void proc_ld(void) {
    ...
    if (get_cpu_ctx()->cur_inst->mode == AM_HL_SPR) {
        u8 hflag = (cpu_read_reg(get_cpu_ctx()->cur_inst->reg_2) & 0xF) + 
            (get_cpu_ctx()->fetched_data & 0xF) >= 0x10;

        u8 cflag = (cpu_read_reg(get_cpu_ctx()->cur_inst->reg_2) & 0xFF) + 
            (get_cpu_ctx()->fetched_data & 0xFF) >= 0x100;

        cpu_set_flags( 0, 0, hflag, cflag);
        cpu_set_reg(get_cpu_ctx()->cur_inst->reg_1, 
            cpu_read_reg(get_cpu_ctx()->cur_inst->reg_2) + (int8_t)get_cpu_ctx()->fetched_data);
...
void proc_ldh(void) {
    if (get_cpu_ctx()->cur_inst->reg_1 == RT_A) {
        cpu_set_reg(get_cpu_ctx()->cur_inst->reg_1, bus_read(0xFF00 | get_cpu_ctx()->fetched_data));
    } else {
